...
$(document).ready(function () {
  $(document).on("click.woEvent", ".question-option .icon", function () {
    ...
    const hasSelectedAnswer =
      $(this).closest(".question-item").find(".question-option.is_selected")
        .length > 0;
  ...
  $(document).on(
    "click.woEvent",
    ".submit-btn:not(.not-allowed)",
    async function () {
      showConfirmWithNotice(
        "Xác nhận chấm điểm.",
        "Chấm điểm",
        "Nếu bạn đã làm bài tập này trước đó và lần chấm điểm này có điểm số thấp hơn thì bài làm của bạn sẽ không được lưu.",
  ...
  $(document).on("click.woEvent", ".reset-btn", function () {
    showConfirm(
      "Xác nhận loại bỏ những thay đổi của bạn.",
      "Xác nhận",
      function (result) {
        if (result) {
            ...
            <div class="row gap-24 full-width">
                <div class="overview__container col">
                    <div class="preview-questions_container col gap-8 panel full-width sticky-top">
                        ${createListPreviewQuestionComponent(
...
async function submitMultipleChoiceExercise() {
  ...
  const { score, last_score, result_id, completed_topic_id } = await userApi(
    "submit-multiple-choice-exercise",
    "POST",
    body
  );

  showResponse(score, last_score, exerciseId, completed_topic_id);
}
...
function showResponse(score, last_score, exerciseId, completed_topic_id) {
  const $container = $(".user__container");
  ...
  if (last_score === null || last_score < score) {
    // nộp bài lần đầu
    message = `Đã hoàn tất chấm điểm, bài làm của bạn đạt ${score} điểm.`;
    mainButton = {
      href: `/system-exercise-result?id=${exerciseId}`,
      text: "Xem bài làm",
      is_main: 1,
    };
  } else {
    message = `Đã hoàn tất chấm điểm, bài làm của bạn đạt ${score} điểm, ${ last_score > score ? "thấp hơn" : "bằng với" } lần nộp bài trước đó.`;
    mainButton = {
      href: `/system-exercise-result?id=${exerciseId}`,
      text: "Xem bài làm trước đó",
      is_main: 1,
    };
  }
  ...
  $container.empty().append(
    createAlertSuccessComponent(message, [
      {
        href: `/system-exercise-topic?name=${topic.name}&id=${topicId}`,
        text: "Quay về trang chủ đề.",
        is_main: 0,
      },
      mainButton,
    ])

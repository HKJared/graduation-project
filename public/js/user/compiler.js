...
$(document).ready(function () {
  ...
  // Tính dòng cuối cùng và ký tự cuối cùng
  let lastLine = editor.lineCount() - 1; // Dòng cuối cùng
  let lastChar = editor.getLine(lastLine).length; // Ký tự cuối cùng trong dòng đó
  editor.setCursor(lastLine, lastChar); // Đặt con trỏ tại vị trí cuối cùng
});
...
async function runCode() {
  ...
  runBtn.html(
    '<ion-icon class="running-icon" name="color-filter-outline"></ion-icon>'
  );
  ...
  try {
    ...
    if (data.executionTime) {
      $(".execution-time-result")
        .empty()
        .append(
          `<div class="row gap-8 success item-center"><ion-icon name="stopwatch-outline"></ion-icon> ${data.executionTime}ms</div>`
        );
    } else {
      $(".execution-time-result")
        .empty()
        .append(
          `<div class="row gap-8 danger item-center"><ion-icon name="alert-circle-outline"></ion-icon> Error</div>`
        );
    }
  } catch (error) {
    outputEditor.setValue(
    ...
    const codeMirrorScroll =
      codeMirror.getElementsByClassName("CodeMirror-scroll")[0];
...
function createNewFile() {
  ...
  if (currentCode != "" && !Object.values(sampleCode).includes(currentCode)) {
    showConfirm(
      "Xác nhận hủy bỏ nội dung file hiện tại.",
      "Xác nhận",
      function (result) {
        if (result) {
...
function downloadFile() {
  ...
  if (!fileContent) {
    showStackedNotification(
      "Vui lòng nhập nội dung vào editor trước khi tải xuống.",
      "no_file_name"
    );
    return;
  }

...
const { validateExerciseData } = require("../../validators/topicExerciseValidator");
...
class ExerciseController {
    ...
    static async createExercise(req, res) {
        try {
            ...
            if (!topic.is_editable) {
                ...
                return res.status(400).json({ message: 'Chủ đề hiện không thể chỉnh sửa và tạo bài tập mới.' });
            }
            ...
            if (!exercise_id) {
                ...
                await LogModel.updateDetailLog('Thêm bài tập mới vào db không thành công.', log_id);

                return res.status(400).json({ message: 'Thêm bài tập mới không thành công, vui lòng thử lại hoặc tải lại trang.' });
            }

            if (exercise.type == 'multiple_choice') {
                await ExerciseModel.createMultipleChoiceExercises(exercise_id, multiple_choice_exercises);
            } else {
                await ExerciseModel.createCodeExercise(exercise_id, code_exercise)
            }
            
            await LogModel.updateStatusLog(log_id);
            await LogModel.updateDetailLog(`Tạo bài tập thành công với ID: ${exercise_id}`, log_id);
            return res.status(201).json({ message: "Tạo bài tập thành công.", exercise_id: exercise_id });
        } catch (error) {
            console.error(error);
    ...
    static async getExerciseByUser(req, res) {
        try {
            ...
            if (!exercise_id) {
                await LogModel.updateDetailLog('Không có ID của bài tập được cung cấp.', log_id);
                return res.status(404).json({ message: 'Không tìm thấy bài tập, vui lòng thử lại hoặc tải lại trang.' })
            }

            await LogModel.updateDetailLog(`Làm bài tập có ID: ${ exercise_id }.`, log_id);

            const exercise = await ExerciseModel.getExerciseById(exercise_id);

            if (!exercise) {
                await LogModel.updateDetailLog('Không tìm thấy bài tập trong DB.', log_id);
                return res.status(404).json({ message: 'Không tìm thấy bài tập, vui lòng thử lại hoặc tải lại trang.' })
            }

            if (exercise.is_editable) {
                await LogModel.updateDetailLog('Chủ đề của bài tập này đang trong giai đoạn chỉnh sửa.');
                return res.status(404).json({ message: 'Không tìm thấy bài tập, vui lòng thử lại hoặc tải lại trang.' })
            }

            if (exercise.type == "multiple_choice") {
                const questions = await ExerciseModel.getMultipleChoiceExercisesByExerciseId(exercise_id);
                ...
                function getRandomQuestions(questions) {
                    ...
                    const additionalQuestions = getRandomElements(notRequired, remainingCount);
                ...
                exercise.code_exercise = await ExerciseModel.getCodeExercise(exercise_id);
            }
    ...
    static async getTopicExercisesByAdmin(req, res) {
        try {
            ...
            if (!topic_id) {
                await LogModel.updateDetailLog('Không có id chủ đề được cung cấp.', log_id);

                return res.status(400).json({ message: 'Không có ID chủ đề được cung cấp.' })
            }

            await LogModel.updateDetailLog('Lấy danh sách bài tập của chủ đề có Id: ' + topic_id, log_id);
    ...
    static async submitMultipleChoiceExercise(req, res) {
        try {
            ...
            if (!exercise_id || !multiple_choice_answers) {
                await LogModel.updateDetailLog('Dữ liệu không được cung cấp (Id bài tập và bài làm).', log_id);

                return res.status(404).json({ message: "Lỗi đường truyền, vui lòng thử lại hoặc tải lại trang." })
            }
            ...
            for (const answer of multiple_choice_answers) {
                ...
                if (!question) {
                    questionResults.push({ question_id: answer.question_id, selected_options: [], is_correct: false });
                    continue;
                }
                ...
                questionResults.push({ question_id: answer.question_id, selected_options: userOptions, is_correct: isCorrect });
            ...
            const last_submit = await ExerciseModel.getUserExerciseResultByExerciseId(user_id, exercise_id);
            if (last_submit && last_submit.id) { // Nếu đã từng làm
                result_id = last_submit.id;
                await LogModel.updateDetailLog(`Điểm bài làm cũ: ${ last_submit.score }. Điểm bài làm mới: ${ score }`, log_id)
                
                last_score = last_submit.score;

                if (score > last_score) {
                    await ExerciseModel.deleteUserMultipleExerciseAnswers(last_submit.id);
                    await ExerciseModel.createMultipleChoiceExerciseAnswers(last_submit.id, questionResults);
                    await ExerciseModel.updateUserExerciseResult(last_submit.id, score, is_completed);
                }
            } else {
                await LogModel.updateDetailLog(`Điểm bài làm mới: ${ score }`, log_id)

                result_id = await ExerciseModel.createUserExerciseResult(user_id, exercise_id, score, is_completed);

                await ExerciseModel.createMultipleChoiceExerciseAnswers(result_id, questionResults);
            }

            // Cập nhật trạng thái hoàn thành của topic
            let completed_topic_id = null;
            if (is_completed) {
                const completed_topic = await TopicModel.getUserCompletedTopic(user_id, exercise.topic_id);
                if (!completed_topic){    
                    const topic = await TopicModel.getTopicById(exercise.topic_id);
                    const user_exercise_results = await ExerciseModel.getUserExerciseResultsByTopicId(user_id, exercise.topic_id);
                    ...
                    if (total_scores >= topic.min_required_score && total_success_exercise >= topic.min_required_exercises) {
            ...
            for (let i = 0; i < test_cases.length; i++) {
                await new Promise((resolve) => {
                    compileCode(
                        ...
                        (result) => {
                            if (result.output.trim() == test_cases[i].output) {
                                totalCorrect++;
                            }
